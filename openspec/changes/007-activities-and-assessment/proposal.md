# Change: 007 Activities and assessment
...
- PWA runtime: IndexedDB replica, service worker, storage budget and
  ...
  labelling.
- Attempt lifecycle facts, on-device marking with the TypeScript engine worker, time-lock key
...
- Automerge drafts and group workspaces (ACT-20).

## Impact

- Adds `slot.v1` to the fact registry; extends subject kinds of attempt and
  response facts to groups (`G`).
- Produces the facts that 008 folds and 010/011 project.
...
002 (device SDK and sync), 003 (sessions, roles, groups), 005 (bundles,
keys, policies, deadlines), 006 (engine), 004 (client shell). Instructor-facing marking
queues read views from 008; until then the marking UI works from the
instructor device's own pull of cohort facts (FLS-10).

## Out of scope

Forum and messaging activities (009); LTI-launched activities (010).
