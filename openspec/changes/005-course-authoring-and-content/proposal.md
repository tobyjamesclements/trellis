# Change: 005 Course authoring and content
...
- `struct-updater` (Java, with the Automerge Java binding) consuming the FIFO `struct` queue; snapshot,
...
- `publish-prepare` (Java) building snapshots, key documents, marking
...
- Automerge document editing for page, book, item and rubric bodies
  (CAC-22) with the client editor from change 004.
...
- Provides the blobs and versions 006 and 007 consume.
...
uses the engine profile from 006; until 006 ships, classification runs
against the static profile document.

## Out of scope

Common Cartridge import/export (011); LTI deep-linking flows (010); CASE
framework import (012).
