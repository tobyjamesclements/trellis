...
- **Push emitter.** Same shape as the AGS emitter (010): home-region check,
...
- **Caliper projector.** Runs in every region (cheap, deterministic) so a
  ...
  assembly in a Java function; JSON-LD contexts embedded; batching by
  `(tenant, 30 s window, 100 events)`. GradeEvents from `effect.ready`
  kind `caliper_grade` produced by the intent evaluator (008).
- **Caliper endpoint.** Bearer tokens from DIO-19; actor resolution by
  pairwise `sub` mapping (`CALMAP#`) populated on LTI launches (010).
...
- **Epoch.** Registry field `epoch` per tenant, bumped by 015's failover
