...
2. **Transport spikes.** Signalling-free WebRTC to an ICE-lite box; Firefox hash pinning; the service-worker address-space check. Their outcomes decide how much the certificate path still matters. *Interim findings from the container run of tasks 1.2, 1.5, and 1.6 (`spikes/`, September 2026), pending the rig:* hash-pinned WebTransport and signalling-free WebRTC both open from Chromium 141 to a bare address with no CA and no signalling, in tens of milliseconds, and both pass the automerge-repo adapter acceptance suite with a Chromium round trip. WebRTC needs werift as the box (ICE-lite mode, static credentials set by hand, the device's certificate accepted for application-layer binding); node-datachannel cannot play the box. Node's WebTransport server is a native addon; certificate rotation restarts its listener. The certificate path (task 1.10, `spikes/acme-dns01/`) issues and renews unattended over DNS-01 through an acme-dns-style registry endpoint against the Pebble test CA; the box must keep its ACME account URL beside the account key in the recovery bundle. Firefox, Safari, Edge on the laptops, Chrome's Local Network Access behaviour, and the real CA and zone remain unmeasured.
...
7. **Box runtime packaging.** Bun compile versus Node single-executable; decide on Pi memory footprint, Automerge WASM start-up, and which WebRTC library builds cleanly. *Interim findings from task 1.7's container run (`spikes/capacity/`), pending the Pi:* on the same workload Bun signed six times slower and folded three to four times slower than Node, its Web Crypto gained nothing from batching, and its peak memory was higher; the WebRTC library that works (werift) is pure TypeScript and builds anywhere, while the WebTransport server is a native addon whose packaging in a single executable is the remaining factor. The same run showed that one log document must be sealed into epochs well before 25,000 records (a 200,000-record document cannot be loaded at all, and every append and storage write grows with the document), and that text documents should be budgeted around 100 KiB.
