...
This capability is the TypeScript and React client every persona uses, from
learner to platform operator: the monorepo package set, the design system,
the progressive web app shell that hosts the learner runtime of ACT-02,
ACT-03 and ACT-04, the Web Worker that hosts the derivation engine (DRV-02),
local replica protection, internationalisation, performance budgets, client
telemetry, the update and hosting pipeline, content rendering security, and
the accessibility conformance that ADR-025 makes a release gate. It owns no
facts and no server items; its only server-side code is one telemetry
ingest function and the static hosting pipeline.
...
1. **Facts.** The profile's own streams (dense `seq`, allocated in the same
   ...
   streams (FLS-10), including Automerge changes (FLS-21); they leave the
   device only after the roster confirms them (ACT-03).
2. **Derived local views.** The engine worker's outputs over the profile's
   facts, bundles and keys, plus the last pulled region rows with their
   cursors; all recomputable. Which source a displayed value comes from is
   decided by dominance (ACT-17, MVA-05, MVA-10), never by a TTL or an age.
3. **Presentation state.** Focus, open dialogs, scroll position, unsaved
   keystrokes; never the only copy of anything a person did.

React state holds only the third kind; anything that must survive a reload
is a fact, a draft awaiting its fact (ACT-01), or a regenerable cache. The
client tolerates any region (FLS-18), any staleness (MVA-02) and any server
engine version (FLS-15, DRV-14), and never withholds or reorders learner work.
...
bundles    content-addressed blobs, LRU within the ACT-02 budget (CAC-14); keys: released, by key_version
...
keyring    wrapped replica key and role-policy wrapping key (UIX-09); telemetry: bounded scrubbed buffer
```

Derived local views (`LocalLearnerView` per module with provenance and
`source: device | region`, `LocalCalendar`, `LocalInbox`, `LocalDoc`) are
pure functions of these stores and the engine version.
...
ConformanceRecord per build_id: axe and manual results per assistive-technology pairing, known issues
                  [{id, wcag_sc, severity, surfaces, workaround, target_build}], reviewed_at (blob)
Statement         per tenant from the ConformanceRecord, tenant.v1.a11y_contact and registered LTI
                  tools with attestations; https://<tenant-host>/accessibility
...
| 3.3.9 Accessible Authentication (Enhanced); 2.2.5 Re-authenticating | AAA, met by construction | No cognitive test exists in any sign-in path; nothing is lost across re-authentication because every saved answer is a fact, and time limits are advisory and extendable (`extension.v1`, ACT-06) |
...
| `uploadInteraction`, `endAttemptInteraction` | File input with progress and result announced; button | — | ready |
| `mediaInteraction` (delivery only) | Media element with captions and transcript from the catalog | — | ready |
| `graphicOrderInteraction`, `graphicAssociateInteraction`, `graphicGapMatchInteraction` | Hotspot or gap buttons on the image plus an ordered list, a pair list or a combobox per gap | Select, then place | pending |
| `selectPointInteraction` | Image with coordinate fields, arrow-key cursor with `aria-valuetext`, zoom | Coordinate entry | pending |

A `pending` status is versioned with the engine profile and flips to `ready`
only through a release that passes the UIX-06 gate.

### New fact types

None. The client writes only types registered to other capabilities
(`device.v1`, `noop.v1`, `void.v1`, `profile.v1`, `consent.v1`, `push.sub.v1`,
the ACT, COM, CAC and CRD types, and `am.change.v1` per FLS-21) through
`POST /sync/v1`; client telemetry is not a fact (UIX-12).
...
React; component state holds presentation state only, and a lint rule fails
any module that persists application state outside the `sync` stores.
...
contrast is checked in CI (4.5:1 text, 3:1 components, 7:1 high contrast),
spacing and type are `rem` tokens, and motion tokens collapse to zero under
`prefers-reduced-motion`. Themes `light`, `dark` and `high_contrast` follow
`a11y.theme` or the system, and every component honours `forced-colors`.
Each component ships an accessibility contract (keyboard table, roles and
states, announcements, RTL and forced-colour behaviour, target size) and an
axe-tested story, or the build fails.
...
3.2.6, 3.3.7, 3.3.8) and adopts 2.4.13 and 3.3.9 although they are AAA.
EN 301 549 clauses 9, 11 and 12 structure the per-release conformance
report, and a public-sector tenant's duty under the 2018 Regulations is met
with the UIX-07 statement.
...
Every function is keyboard-reachable without traps or positive `tabindex`,
focus order follows reading order behind skip links and landmarks, and names,
roles and states follow the ARIA Authoring Practices. Layouts reflow at 320
CSS px with no two-dimensional scrolling except data tables and images,
survive the 1.4.12 text-spacing overrides, and drop non-essential animation
under `prefers-reduced-motion`. Every status (late, in transit, hole,
provisional, over time, override, device versus region source) is icon plus
text, never colour alone. Sync state and MVA-02 freshness changes are
announced through one polite `status` region, coalesced to one announcement
per 5 s per kind and phrased as the freshness block; errors use an `alert`
region; a value changing source is announced and focus never moves.
...
- **THEN** the status region announces "includes work synced up to 09:14; 2 devices have work in transit" once, the focused cell is unchanged, and the two rows gain an "updated" text marker
...
modality produced it (ACT-19). Catalog alternatives (`qti-catalog-info` via
`data-catalog-idref`: spoken audio, glossary, braille text, sign-language
video, simplified language) render per `a11y.catalog`, and item `xml:lang`
and `dir` reach the DOM (UIX-10). At publish, `publish-prepare` records
`accessibility = ready | pending` per item exactly as CAC-08 records
`marking`; `pending` interactions are absent from the authoring palette,
listed in the prepare report for imported items, excluded from delivery and
shown on the device as an accessible placeholder.
...
Chromium, Firefox and WebKit on each pull request with zero violations to
merge; no rule is disabled globally, and a per-instance exemption needs a
linked defect and expires at the next release. The per-release manual plan
covers NVDA with Firefox, JAWS with Chrome, VoiceOver with Safari on macOS
and iOS, TalkBack with Chrome on Android, a keyboard-only pass, a 400% zoom
pass and a forced-colours pass over scripted tasks per persona, recorded in
the ConformanceRecord. Research sessions with disabled learners and
instructors run at least twice a year across visual, motor, cognitive and
hearing needs; findings enter the tracker at severity-1.
...
`app-release` renders `https://<tenant-host>/accessibility` for every tenant
from the build's ConformanceRecord and the tenant's `tenant.v1` fields in the
structure of the UK model accessibility statement: compliance status,
non-accessible content with WCAG criteria and target builds, content outside
scope, third-party content (registered LTI tools with any accessibility
attestation on the registration; authored content checked by CAC-16), testing
approach and dates, the contact route (`tenant.v1.a11y_contact`) and the
enforcement procedure; an EN 301 549 conformance report comes from the same
record. The page is linked from every route's help position (3.2.6),
regenerated on every release and tenant-field change, and retained by `build_id`.
...
The shell posts derivation requests (facts for a `(subject, scope)`, item,
key, rubric and policy blobs by hash, seed inputs) to a module worker
running `@trellis/engine` and receives outputs with provenance (DRV-15); the
worker imports no DOM, `Intl` or `Date` API and the DRV-03 lint runs in CI.
The golden-vector suite (DRV-02) runs against the built worker bundle in
Chromium, Firefox and WebKit and under GraalJS in the server build; any
difference fails the release. The bundle is capped at 300 KB compressed and
the DRV-17 workload must mark within 200 ms on the reference device
(UIX-14), measured per release on real devices. The shell waits at most
500 ms for a `client_mark` before writing the `resp.v1` without one
(ACT-04), so a slow or crashed worker delays feedback but never the fact.
...
`navigator.storage.persist()` and shows the outcome; bundles evict LRU within
60% of quota or 2 GB, released keys next, facts, vectors and drafts never
(ACT-02). P-class bodies, drafts, documents and pulled rows are encrypted
with AES-GCM under a per-profile replica key generated non-extractable in
WebCrypto and stored wrapped in `keyring`. For learner and consumer profiles
the wrapping key persists with the profile so offline work continues
indefinitely; the replica is the learner's own data and its confidentiality
on a lost device rests on the device lock, the accepted risk SEC-09 records.
For instructor, tutor, admin and operator profiles the wrapping key is bound
to the session (IDE-03): created at sign-in and destroyed, with every pulled
fact, row and bundle of other subjects, on sign-out, IDE-12 revocation,
refresh failure, or after `tenant.v1.idle_purge_after` without interaction
(default 12 h for instructor and tutor, 1 h for admin and operator); the
profile's own unpushed facts stay in the outbox until the roster confirms
them (ACT-03). Switching profile closes the database and drops every key.
...
(ACT-15); `engine` and `sync` never import `Intl` or receive a locale
(DRV-03). Item, page and post language (`xml:lang`, `lang`) propagates to
the DOM so assistive technology switches voice (3.1.1, 3.1.2).
...
worker (ACT-03) and the main thread never awaits the network. CI enforces
size limits per entry and chunk, Lighthouse assertions on learner routes and
the UIX-08 worker benchmark; a regression blocks merge.
...
frames, Web Vitals and the UIX-11 timings, sync and worker durations, storage
estimates, and coarse context (`build_id`, `engine_version`, route template,
device class, browser family and major, locale, `tenant_id`,
`hash(device_id, build_id)`); never a fact body, item text, response, name,
address or URL parameter, and a CI schema test fails any field admitting free
text. Events are buffered in IndexedDB and flushed on the sync cadence to
`POST /telemetry/v1` on the tenant's regional API only when the profile's own
folded `consent.v1` grants `analytics` (ADM-16); errors are sampled at 100%
capped at 20 per session, performance at 10% of sessions, guests (IDE-10)
send nothing, and no third-party script, beacon, font or CDN loads on any
Trellis origin (UIX-18). `telemetry-ingest` re-checks consent, validates the
schema, emits EMF metrics and sampled logs (ADM-08) and writes no table item.
...
Builds are uploaded under `/app/<build_id>/` with content-hashed file names
and `Cache-Control: public, max-age=31536000, immutable`; the entry documents
and `sw.js` are served `no-cache`, so a launch discovers a new build, which
the service worker precaches and activates only when no client has an open
attempt or an editing session mid-change, after an accessible "update ready"
notice. IndexedDB migrations are forward-only and run before activation;
rollback re-points the entry document at an earlier `build_id`. The client
marks with its bundled engine and records `engine_version` (DRV-14). The
fact-type versions it emits are pinned to the registry version the server
last confirmed in the sync response's `advice.versions` (an FLS-08 advice
entry carrying the server `engine_version` and registry version), so a
client that ships `resp.v2` keeps emitting `resp.v1` until confirmation
rather than having its work stored uninterpreted (FLS-15).
...
Tier 1 (CI matrix and release gate): current and previous major versions of
Chromium-based Chrome and Edge on desktop and Android, Firefox current and
ESR, Safari current and previous major on macOS and iOS, Samsung Internet
current; minimum platforms Android 10, iOS 16, Windows 10, macOS 12 and
ChromeOS; Tier 2 (manual per release): Firefox on Android and
...
Without a service worker or IndexedDB the app runs online-only with a
persistent notice, an ephemeral `device_id` per page load and immediate push
of every fact. Native wrappers (an Android Trusted Web Activity, an iOS
WKWebView shell) load the same served bundle from the same origin, add only
install, push and deep-link plumbing, hold no business logic or storage, and
pass the same gates with platform assistive technology (UIX-06).

#### Scenario: Private browsing
- **WHEN** a learner opens the app in a private window
- **THEN** it runs online-only, shows the notice that unpushed facts do not survive a reload, registers an ephemeral `device_id` (FLS-19), pushes each fact immediately, and marks locally without persisting a replica
...
sign-up and sign-in by magic link or social and institutional OIDC (IDE-02),
the OpenID Provider consent screen naming the organisation and the claims
released (IDE-20), personal profile and `a11y.*` preferences, personal
learning and the credential backpack (CRD-16), self-service creation of a
soft organisation under the zone's wildcard host with the creator as first
admin (ADM-22, ADM-23), joining by invitation link, code or email invite
(IDE-21) with plain statements of what the organisation can see and do, and
leaving with the portability options of IDE-24. A soft organisation's admin
surface manages membership and roles only and says why identity controls are
absent (`409 identity_owned_by_realm`); a strict organisation's (IDE-22)
manages creation, suspension, deprovisioning and erasure with provisioning
status (console, SCIM 2.0 DIO-20, OneRoster DIO-02, IdP), the external-guests
policy (IDE-23), hostname and SSO, never offers a password reset (IDE-02),
and states what each terminal action destroys (ADM-11).
...
Gradebooks (MVA-06), marking queues (ACT-12), slot rosters (ACT-09), at-risk
flags (MVA-09), rankings (MVA-18) and analytics (MVA-13) render the freshness
block in its MVA-02 meaning, mark rows with pending streams and holes
(MVA-17), show the vector digest beside every non-monotone value (MVA-07),
and never a bare timestamp or an age. After the instructor's own write the
client polls `POST /views/{partition}/dominates` (MVA-05) until the row
cursor includes its stream's `seq`, showing "your change is not yet
...
(known-tensions §2); `freshest=true` (MVA-11) is an explicit control labelled
with the serving region, and the MVA-14 banner is a status region.
...
learners (ADR-023, ACT-10). Each local change becomes one `am.change.v1`
fact through `sync` (actor = `device_id`, per-actor `seq` = stream `seq`,
deps in `refs`, FLS-21) on a 2 s idle or blur cadence, and the document is
materialised in a dedicated worker from the cached snapshot for its heads
plus later changes. The editor is the design-system multiline textbox with
an APG toolbar, announcing marks and structure. Concurrent values of one map
key appear as a conflict chip listing each value with author and sync time
and are resolved by choosing, which is a new change; duplicated list
elements from concurrent moves are de-duplicated with a history note, and
history lists every change by author, device and `sync_hlc`.
...
HTML from blobs and facts (page and book bodies, item stems, feedback, posts)
passes through the design-system sanitiser (allowlisted elements, attributes
and URL schemes; MathML allowed; SVG restricted; `style` limited to a token
allowlist; resources only from the content origin), enforced by a Trusted
Types policy where supported and a lint that bans any other HTML sink. The
app origin's CSP is `default-src 'self'; script-src 'self' 'wasm-unsafe-eval'`
(the Automerge core) with no `'unsafe-inline'` or `'unsafe-eval'`,
`connect-src` limited to the tenant's API and content origins, `object-src
'none'`, `base-uri 'none'` and `frame-ancestors 'none'` (tool-role launch
pages served by the API set per-registration `frame-ancestors`, LTI-05),
with subresource integrity on the entry scripts. Blobs and user files are
served from the zone's content distribution on a separate origin (CAC-03)
with `nosniff` and `Content-Disposition: attachment` for non-renderable
types. LTI tools (LTI-01, CAC-18) open in a new window by default with a
warning; a registration that asks for embedding gets an `iframe` with
`sandbox="allow-scripts allow-forms allow-same-origin allow-popups"`, a
minimal `allow` list and a `src` origin matching the registration, and
`postMessage` handlers validate the origin against it.
...
The client owns no item in `facts`, `derived` or `registry`; it appends facts
only through `POST /sync/v1` (FLS-04, FLS-11), reads server state only through
each owner's regional API, and never holds table credentials. It reads:
...
| Memberships, roles, profile, directory (IDE); consent, config, flags (ADM) | `derived` `T#t#E#<usr>`, `T#t#RL#<usr>`, `T#t#PR#<usr>`, `T#t#DIR`; `T#t#CN#<subject>`, `T#t#CFG` | Session `roles_hint`, `403` responses, profile and directory APIs (IDE-04, IDE-09, IDE-13); own `consent.v1` facts, config endpoint (ADM-05, ADM-16) | Surface selection, profile, admin search; telemetry gate; flags hide surfaces only |
| Inbox, threads (COM) | `derived` COM items | Inbox and thread APIs (COM-04, COM-17) | Cached pages with freshness |

**Sizing and hot partitions.** A learner replica holds ~2,400 facts a year
(~3 MB at 1.2 KB), rows of ~4 KB per module and bundles up to the ACT-02
budget; an instructor's cohort replica for one module at the cohort bound is
2,000 × ~40 facts × 1.2 KB ≈ 96 MB, pulled per cohort with pagination for
the modules the instructor takes offline. The client has no hot partition of
its own; a synchronised launch of 2,000 devices is ~35 API requests/s per
cohort, inside FLS-13 and ADM-04 bounds, the static part lands on
CloudFront, and background sync and prefetch are jittered by 0–30 s.
...
There are no other functions; every API the client calls belongs to the data's owner.

## Propagation path

1. A user action becomes a fact: `sync` allocates the next `seq` and stores
   the fact in one IndexedDB transaction (FLS-03); drafts wait in `drafts`
   (ACT-01); Automerge edits become `am.change.v1` facts (UIX-17).
2. The engine worker folds the fact into the profile's local view (UIX-08),
   the document worker updates `LocalDoc`, and React re-renders from the
   derived view labelled "computed on this device" (ACT-17).
3. The service-worker sync agent pushes the outbox in `seq` order, pulls by
   vector, applies roster advice, uploads blobs, fetches bundles from
   CloudFront and released keys from `GET /keys/release` (ACT-03, CAC-14,
   ACT-05), and flushes consented telemetry to `telemetry-ingest` (UIX-12),
   never on the interaction path.
4. Pulled facts and rows fold into the stores; dominance decides row versus
   local derivation per value (MVA-05, ACT-17); the status live region
   announces the freshness change (UIX-04).

Server side: CI pipeline → `app-release` → S3 `/app/<build_id>/` → CloudFront
→ service-worker install and activation (UIX-13); `tenant.v1` folds carrying
`a11y_*` fields → EventBridge `fact.folded` → `app-release` → regenerated
statement (UIX-07). No Streams, SQS or Step Functions are used here.
...
| Build storage, invalidations, statements, `app-release` | 24 builds × 8 MB retained a year ≈ 2.3 GB × $0.023 + 12 k PUTs × $0.005/k; invalidations ≤ 1,000 paths free †; statements 2 × 200 KB; 2 releases + ~50 tenant-field folds × 3 s × 0.5 GB → **$0.1** | $0.3 | 100 tenants at 1M |
...
1. **Drag-and-drop QTI interactions versus 2.5.7.** Where it breaks: order,
   ...
   interactions are pointer-driven in most delivery engines, and 2.5.7 needs
   a single-pointer alternative for every drag. Symptom: an author wants an
   interaction the palette does not offer. Options: (a) select-then-place and
   keyboard paths so the drag is an enhancement; (b) hide interactions until
   their rendering ships (`accessibility = pending`); (c) `server_only`
   marking, which changes nothing about rendering and is rejected.
   Recommendation: (a) for every `ready` interaction, (b) for the four graphic
   and point interactions until a release passes the gate (T-48).
2. **GraalJS versus browser engine determinism.** Where it breaks: one
   TypeScript engine runs on four JavaScript engines; `Math`, `Intl`, `Date`
   and regular-expression corners differ between them. Symptom: a device mark
   differs from the server mark for the same facts. Options: (a) the
   lint-enforced subset plus the golden-vector matrix on every release; (b)
   server-only marking for constructs that ever diverged; (c) the TeaVM
   fallback of ADR-001. Recommendation: (a); the residual is detected by
   DRV-14 and the device shows the server value on convergence (ACT-04).
3. **Offline data on shared devices.** Where it breaks: a classroom tablet
   ...
   navigation (ACT Known Tension 5); (c) OS-level accounts per learner,
   outside the product. Recommendation: (a), with (b) for classroom devices.
4. **Accessibility of third-party LTI tools in iframes.** Where it breaks:
   a tool's UI is the vendor's, cannot be gated, and inherits none of the
   design system. Symptom: a conformant LMS embeds a non-conformant activity.
   Options: (a) the statement discloses embedded tools and the registration
   carries the vendor's accessibility attestation shown to authors (T-49);
   (b) open tools in a new window by default so assistive technology treats
   them as separate pages (UIX-18); (c) a tenant flag refusing tools without
   an attestation. Recommendation: (a) and (b) always, (c) as an opt-in; the
   statement must say the guarantee cannot be given.
5. **Performance budgets versus Automerge and engine bundle sizes.** Where
   it breaks: the engine (300 KB) and the Automerge core (~0.7 MB compressed
   †) together exceed the 200 KB learner shell several times over. Symptom:
   slow first load on the reference device if either is in the critical
   path. Options: (a) load the engine after first paint and Automerge only on
   document routes, reading published pages from the materialised bundle blob
   rather than the document; (b) a lighter single-device draft
   representation, losing multi-device merge; (c) raise the shell budget.
   Recommendation: (a) with per-route budgets in CI (UIX-11); (b) and (c)
   rejected.
