...
- **Facts (global, immutable):** `framework.v1`, `align.v1`,
  ...
  `award.v1`, `issue.v1`, `revoke.v1`, `consent.v1`; union is the merge and
...
- **Derived (regional, disposable):** framework, alignment, achievement and
  ...
  credential-id maps; status-list state; OAuth nonces; all vector-stamped
  and regenerable from facts and blobs (ADR-004).
...
- **Side effects (home region only, sent ledger, ADR-012):** credential and
  ...
  DID document is a derivation any region performs, while signing a
  credential is a side effect because the artefact is handed to a holder.
- **Where the model is weakest:** non-monotone award predicates (hence the
  24 h window, MVA-08), issuance undone only by revocation, double issuance
  during a flip, and revocation reaching a peer only after replication.
...
`revoke.v1` writer also "region system device (home)" (CRD-13, CRD-14).

```
Framework    framework.v1 → CFPackage blob → per-region index generation (CFItems with parent,
               children[], fullStatement, humanCodingScheme, CFItemType; CFAssociations; search tokens)
Alignment    align.v1 { target:{kind: item|activity|module, id}, cf_item:{framework, sourcedId,
               uri}, relation: assesses|teaches, weight? }
Achievement  achievement.v1 { achievement_id, achievement_version = hash(body), name, criteria,
               image_ref, achievementType, alignment[], validity?, issuer_id, certificate_template_ref?,
               rule: manual | mastery{cf_items, require: all|any} | completion{scope}
                   | grade{scope, min} | plan_completed, on_predicate_loss: flag|revoke|keep }
Evidence     evidence.v1 { cf_item?, achievement_id?, kind: file|url|text|attempt, body_ref?, url?,
               name, description, narrative, share: private|credential, rating? (instructor only) }
Award        award.v1 { kind: rule|manual|reissue|clr|affirm, achievement_id, achievement_version,
               issuer_id, intent_id?, predicate_inputs?, evidence_refs[], supersedes?, reason? }
...
| 5 | Sent ledger | `T#t#LEDGER#<region>#crd` | `award#<intent_id>`, `credential#<award_fact_id>`, `clr#<award_fact_id>` | Region-owned, channel shard per FLS pattern 7; read from every region before deciding or issuing |
| 6 | Key registry (read only, ADM) | `T#t#KEYS` | `K#<kid>` | Key group `issuer:<issuer_id>`; states |

### `derived` (regional)

| Item | PK | SK | Size | Access |
|---|---|---|---|---|
| Framework index | `T#t#CRD#FW#<sourcedId>` | `HDR`, `G#<gen>#I#<sourcedId>`, `G#<gen>#A#<sourcedId>`, `G#<gen>#S#<token>` | 1–50 KB | Generation switch by CAS; GetItem per CFItem (children listed in the parent); picker prefix Query (ADR-016) |
...
associations × 0.3 KB + ~10 MB of search tokens ≈ 18 MB per region; an
alignment index: 500 items × 3 alignments × 2 directions × 0.3 KB ≈ 1 MB; a
backpack ≈ 15 KB per learner (150 MB per 10,000); `CID`: 100,000 × 150 B ≈
15 MB per tenant-year; mastery: 2,000 × 2 KB ≈ 4 MB per partition. No LSIs.

**Hot-partition risk.** `T#t#CRD#PEND`, `T#t#CRD#CID` and `SL#…#CTR#<region>`
take one write per issuance on one PK per tenant: 10,000 issuances a month
at L10k is 0.004 writes/s, and a term-end burst of 10,000 intents in one
hour is bounded by the emitter's concurrency (20 × ~4 issuances/s ≈ 80
writes/s per PK), far under the 1,000 WCU/s ceiling and the KMS `Sign`
quota. At 1M a million-intent burst is a throughput problem (≈ 3.5 h at
concurrency 20), not a heat one; concurrency 200 needs `PEND` and `CID`
sharded by `hash(subject) mod 16`. Index builds ≤ 500 writes/s; ledger < 1/s.
...
1. Structure and tenant facts (`align.v1`, `achievement.v1`, `framework.v1`,
   ...
   `fact.folded` → SQS FIFO `crd-index` (group = course or tenant) →
   `crd-index-updater`; `framework.v1` also starts `case-index` per region.
...
6. Flip and apologies: ADM-03 `home.changed` → `credential-emitter` (new
   ...
   `credential.predicate_lost` → Step Functions Standard `credential-reconcile`
   / `credential-review` (home) → decision or timeout → `revoke.v1` or `affirm` → step 5.
7. Anti-entropy repairs (FLS-14) re-enter steps 1 and 5; the hourly
   status-list rebuild covers any event missed on the way.

Telemetry per region (ADM-08): intent deferrals, award-to-ledger issuance
latency, duplicates by case, open reviews, status-list build lag (`now −
max_sync_hlc`), verification outcomes, `oauth.code_reuse`, KMS throttles;
alarms on issuance p99 > 10 min, status-list lag > 5 min, any KMS throttle.
...
| CASE imports (5 per year), reconcile and review workflows | Step Functions ~50 transitions + 15 k WRU × 2 regions per import; ≤ 200 workflows × 10 transitions → **$0.06** | $6 | |
| Blob storage and CRR (VCs 50 MB/month cumulative, frameworks 25 MB, evidence ~5 GB) plus derived storage (~0.4 GB) | 5 GB × $0.023 × 2 + 5 GB × $0.02 + 0.4 GB × 2 × $0.25 → **$0.55** | $55 | Evidence dominates |
...
| Open Badges 3.0 (on W3C VC Data Model 2.0, VC-JOSE, `did:web`, Bitstring Status List, 1EdTech Revocation List) | Issuer | 1EdTech OB 3.0 Issuer certification including the OB3 API: discovery, RFC 7591 registration, authorization code + PKCE, `getCredentials`, `getProfile`; VC-JWT ES256 | CRD-01, CRD-06 to CRD-10, CRD-12 to CRD-14 | Host (`upsertCredential`, `putProfile`): third-party mutable profile state; learners use external wallets. Displayer: foreign-issuer verification cache. Endorsements: third-party keys. Data Integrity `ecdsa-rdfc-2019`: RDF canonicalisation cost, later change. RS256 kept as a tenant fallback pending the certification suite's ES256 support | **Status-list lag:** a revoked credential verifies as valid in a peer region for replication + fold + 60 s cache; the list embeds its vector and the verifier report states it. **Double issuance:** one shared award fact gives one id (dedupe by id); two award facts give two ids, reconciled by a review-and-revoke workflow. **Mastery non-monotone:** awards fire only after a 24 h stability window at a dominating vector; at most one award per `intent_id` per region ledger. **Award reversal after issuance:** nothing un-issues; a lost predicate becomes a flagged review or a policy revocation with a learner-facing explanation |
...
1. **Issuance is irrevocable except by revocation.** Where the model breaks:
   ...
   cannot un-issue it. Symptom: a badge the learner no longer qualifies for,
   then a revocation with an apology. Options: (A) instructor review,
   default keep after 14 days (CRD-14); (B) automatic revocation after a
   second 24 h window; (C) never revoke on predicate loss. Recommendation:
   A by default, B per achievement for compliance-type credentials, and
   every revocation message written for the learner.
2. **The 24 h window delays legitimate badges.** Where the model breaks: the
   ...
   time. Symptom: the badge arrives a day late, and an active learner is
   deferred further by the quiet-period clause. Options: (A) 24 h, quiet
   period scoped to the predicate's inputs, 7-day maximum deferral (CRD-06);
   (B) a shorter tenant window (1 h) for low-stakes micro-credentials; (C)
   issue at once and lean on revocation. Recommendation: A, with B as tenant
   policy per achievement and a 5-minute floor (MVA Tension 3); C is ruled
   out since revocation is an apology, not a correction.
3. **Two notifications during a home-region flip.** Where the model breaks:
   ...
   emails, and in case (b) of CRD-13 two credentials then a revocation.
   Options: (A) deterministic ids, cross-region ledger reads, reconcile
   workflow; (B) pause emission for one replication interval after a flip
   (ADM-03 checks catch-up); (C) COM-07 dedupes notifications by credential
   id. Recommendation: A and B, C as a cheap addition; accept the residual.
...
5. **Deterministic ids and re-award after revocation.** Where the model
   ...
   Recommendation: A now, B as a later change; C is ruled out.
6. **A KMS asymmetric multi-Region key per issuer.** Where the model breaks:
   ...
   (B) one tenant key shared by all issuers, each DID pointing at it; (C) one
   key per zone shared by tenants. Recommendation: A for few issuers, B as an
   admin option for many departments; C is ruled out (cross-tenant blast).
7. **Mastery disagrees across regions and is not sticky.** Where the model
   ...
   in two regions see different competency reports. Options: (A) accept and
   always show the vector and the reason for a transition; (B) record
   mastery as a fact, contradicting rule 1; (C) serve the freshest comparable
   view on request (MVA-11). Recommendation: A with C; B is ruled out.
